...
const configureCache = (options) => {
    ...
    const maxEntrySize = options.maxEntrySize && options.maxEntrySize > 0 ? options.maxEntrySize : 5_000_000;
    const { onCacheHit, onCacheMiss } = options;
    const calcExecTime = (start, end) => {
        const diff = end - start;
        return Number(diff) / 1_000_000; // convert nanoseconds to milliseconds
    };
    // Function to add a query result to the cache
    async function set(queryKey, data, dependencies, ttlInSeconds = defaultTtl) {
        ...
        const compressedData = await (0, snappy_1.compress)(binaryData);
    ...
    async function get(queryKey) {
        ...
        if (!compressedData) {
            ...
            const end = process.hrtime.bigint();
            if (onCacheMiss)
                onCacheMiss(queryKey, calcExecTime(start, end));
            // console.log(`cache miss in ${calcExecTime(start, end).toFixed(3)}`);
            return null;
        }
        // Decompress result
        // const startSnappy = process.hrtime.bigint();
        const binaryData = await (0, snappy_1.uncompress)(compressedData);
        ...
        const end = process.hrtime.bigint();
        if (onCacheHit)
            onCacheHit(queryKey, calcExecTime(start, end));
    ...
    async function invalidate(dependency) {
        // const start = process.hrtime.bigint();
        const dependencyKey = `dependency:${dependency}`;
    ...
    async function getStringKeySize() {
        let cursor = '0';
...
exports.default = exports.configureCache;
